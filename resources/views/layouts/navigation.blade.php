...
<nav x-data="{ open: false }" class="bg-white/95 dark:bg-gray-900/95 backdrop-blur-lg border-b border-gray-100 dark:border-gray-800 shadow-sm sticky top-0 z-50 transition-colors duration-300">
            ...
            <div class="flex items-center gap-8">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-2.5 group">
                        ...
                        <span class="font-bold text-lg text-gray-900 dark:text-gray-100 tracking-tight hidden sm:block">Tortue Blog</span>
                    </a>
                </div>
                ...
                <div class="hidden space-x-1 lg:space-x-2 sm:flex">
                    @auth
                        <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 hover:bg-gray-100 dark:hover:bg-gray-800 dark:text-gray-300">
                            {{ __('Espace Auteur') }}
                        </x-nav-link>

                        <x-nav-link :href="route('public.index', Auth::id())" :active="request()->routeIs('public.index')" class="px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 hover:bg-gray-100 dark:hover:bg-gray-800 dark:text-gray-300">
                            {{ __('Mon Blog Public') }}
                        </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:gap-3">
                ...
                @auth
                    <x-dropdown alig n="right" width="48">
                        ...
                        <x-slot name="content">
                            <div class="py-1">
                                <x-dropdown-link :href="route('profile.edit')" class="flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-100 transition-colors duration-150">
                                ...
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <x-dropdown-link :href="route('logout')" onclick="event.preventDefault(); this.closest('form').submit();" class="flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50 transition-colors duration-150">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                @else
                    <div class="flex items-center gap-2">
                        <a href="{{ route('login') }}" class="px-4 py-2 rounded-lg text-sm font-semibold text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 transition-all duration-200">
                            {{ __('Log in') }}
                        </a>
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="px-4 py-2 rounded-lg text-sm font-semibold bg-gray-900 text-white hover:bg-gray-800 dark:bg-gray-100 dark:text-gray-900 dark:hover:bg-gray-200 transition-all duration-200">
                                {{ __('Register') }}
                            </a>
                        @endif
                    </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900">
